...
type ScopedService = { id: string; name: string; provider_type: string; professional_id: string | null; business_id: string | null; };
type Business = { id: string; name: string; description: string | null; location: string | null; verified: boolean; };
...
export default async function AdminProvidersRoute() {
  ...
  if (serviceIds.length) { const { data, error } = await supabase.from('services').select('id,name,provider_type,professional_id,business_id').in('id', serviceIds).eq('active', true); if (error) throw new Error(error.message); services = (data ?? []) as ScopedService[]; }
  ...
  let businesses: Business[] = []; let professionals: Professional[] = [];
  if (businessIds.length) { const { data, error } = await supabase.from('businesses').select('id,name,description,location,verified').in('id', businessIds); if (error) throw new Error(error.message); businesses = (data ?? []) as Business[]; }
  if (professionalIds.length) { const { data, error } = await supabase.from('professional_profiles').select('id,headline,description,service_area,verified').in('id', professionalIds); if (error) throw new Error(error.message); professionals = (data ?? []) as Professional[]; }
  const businessMap = new Map(businesses.map((business) => [business.id, business])); const professionalMap = new Map(professionals.map((professional) => [professional.id, professional])); const providerMap = new Map<string, ProviderView>();
  for (const service of services) {
    if (service.provider_type === 'business' && service.business_id) { const business = businessMap.get(service.business_id); const key = `business:${service.business_id}`; const existing = providerMap.get(key); if (existing) existing.services.push(service.name); else providerMap.set(key, { key, name: business?.name || 'Business provider', type: 'Business', description: business?.description || 'Business provider in this administrator scope.', location: business?.location || 'Location not set', verified: Boolean(business?.verified), services: [service.name] }); }
