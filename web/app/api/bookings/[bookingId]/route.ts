...
export async function PATCH(request: Request, { params }: { params: Promise<{ bookingId: string }> }) {
  try {
    ...
    if (body.status === 'cancelled') {
      ...
      const booking = await productionBookingRepository.updateBookingStatus(session, bookingId as EntityId, body.status, reason, request);
    ...
    if (body.status === 'rescheduled') {
      ...
      const booking = await productionBookingRepository.rescheduleBooking(session, bookingId as EntityId, { booking_date: body.booking_date, start_time: body.start_time, reason }, request);
      return NextResponse.json({ booking });
    }
