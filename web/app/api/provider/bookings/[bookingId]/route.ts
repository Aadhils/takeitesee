...
export async function GET(request: Request, context: { params: Promise<{ bookingId: string }> }) {
  try {
    ...
    const booking = await productionProviderBookingRepository.getById(session, bookingId as EntityId, request);
...
export async function PATCH(request: Request, context: { params: Promise<{ bookingId: string }> }) {
  try {
    ...
    const booking = await transitionProviderBookingStatus(session, bookingId as EntityId, body.action, body.reason, request);
    return NextResponse.json({ booking });
  } catch (error) {
