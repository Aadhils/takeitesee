...
type Readiness = { marketplace_live?: boolean; services?: Array<Record<string, unknown> & { launch_ready?: boolean }> } & Record<string, unknown>;

export async function GET(request: Request) {
  try {
    const session = await productionAuthProvider.requireProvider(request);
    const supabase = await createSupabaseServerClient();
    const [readinessResult, optionsResult, requestsResult, trustResult] = await Promise.all([
      ...
      supabase.rpc('get_my_provider_trust_state'),
    ]);
    ...
    return NextResponse.json({ readiness, trust, options: optionsResult.data, requests: requestsResult.data ?? [] });
  } catch (error) {
