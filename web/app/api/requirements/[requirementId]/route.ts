...
type Status = 'open' | 'paused' | 'fulfilled' | 'cancelled';
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const [{ data: requirement, error }, { data: events, error: eventError }] = await Promise.all([
      supabase
        .from('customer_requirements')
        .select('id,requirement_reference,customer_id,category_id,location_id,title,description,service_mode,budget_type,budget_min_minor,budget_max_minor,currency,needed_by,status,published_at,closed_at,created_at,updated_at,platform_categories(name,code),platform_locations(name,code,timezone)')
        .eq('id', requirementId)
    ...
    return NextResponse.json({ requirement, events: events ?? [] });
  } catch (error) {
