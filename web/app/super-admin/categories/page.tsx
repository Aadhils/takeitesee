...
export default async function CategoriesPage() {
  ...
  return <main className="container section-stack">
    ...
    <section className="card"><h2><LocaleText en="Add category" ta="வகை சேர்க்க" /></h2><form action={createCategory} className="section-stack">
      <label><LocaleText en="Application" ta="Application" /><select name="application_id" required defaultValue=""><option value="" disabled><LocaleText en="Select application" ta="Application தேர்வு செய்" /></option>{(applications ?? []).map((app) => <option key={app.id} value={app.id}>{app.name} ({app.status})</option>)}</select></label>
      ...
      <label><LocaleText en="Parent category" ta="மேல் வகை" /><select name="parent_id" defaultValue=""><option value=""><LocaleText en="None / root category" ta="இல்லை / root category" /></option>{(categories ?? []).map((category) => <option key={category.id} value={category.id}>{category.name} — {appName.get(category.application_id) ?? 'Application'}</option>)}</select></label>
