...
export default async function SuperAdminPage() {
  ...
  const [applications, providerApplications, providerVerifications, locations, categories, admins, auditEvents] = await Promise.all([
  ...
  return (
    <main className="container section-stack">
      <section className="page-intro">
        ...
        <h1>Super Admin</h1>
        <p>Manage the takeitesee ecosystem across applications, provider onboarding and verification, locations, categories, services, and delegated administrators.</p>
      </section>

      <section className="dashboard-grid" aria-label="Platform overview">
        {metrics.map(([label, value]) => (
          <article className="card" key={label}><span className="eyebrow">{label}</span><h2>{value}</h2></article>
        ))}
      </section>

      <section className="card">
        <h2>Control plane</h2>
        <p>Provider access approval and provider trust verification are separate platform decisions. Public service publishing requires verified ownership.</p>
