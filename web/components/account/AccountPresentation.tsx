...
export function AccountShell({ children, active, customerName = 'Your account' }: { children: React.ReactNode; active: string; customerName?: string }) {
...
export function ProfileSummary({ compact = false, profile }: { compact?: boolean; profile?: CustomerProfile }) {
  const customerName = profile?.displayName ?? 'Your account';
  const initials = customerName.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase();
  return <Card className={`profile-summary ${compact ? 'profile-summary-compact' : ''}`}><div className="provider-avatar provider-avatar-large" aria-hidden="true">{initials || '?'}</div><div><span className="eyebrow">Customer profile</span><h2>{customerName}</h2>{profile ? <><p>{profile.email}</p><span className="card-location">{profile.location || 'Location not added'}</span></> : null}</div></Card>;
}
...
function useAuthenticatedCustomerProfile(): CustomerProfile | undefined {
