...
export default function ProviderApplicationsManager() {
  ...
  return <div className="section-stack">
    ...
    {visible.map((application) => {
      ...
      const oppositeProfile = application.provider_type === 'professional' ? business : professional;
      const requestedProfile = application.provider_type === 'professional' ? professional : business;
      const secondaryRequest = application.status === 'pending' && Boolean(oppositeProfile) && !requestedProfile;
      const staleRequestedOwnership = application.status === 'pending' && Boolean(requestedProfile);
      ...
      return <Card key={application.id}>
        <div className="section-heading"><div><span className="eyebrow">{application.provider_type === 'business' ? t('applications.business') : t('applications.professional')}</span><h2>{application.display_name}</h2><div className="button-row" style={{ marginTop: '.5rem' }}>{secondaryRequest ? <Badge tone="neutral">{text('Secondary profile request', 'இரண்டாவது provider profile request')}</Badge> : null}{staleRequestedOwnership ? <Badge tone="danger">{text('Requested profile already exists', 'கோரிய provider profile ஏற்கனவே உள்ளது')}</Badge> : null}</div></div><Badge tone={tone(application.status)}>{statusLabel(application.status)}</Badge></div>
        ...
        <div style={{ marginTop: '1rem' }}>
          <strong>{text('Existing provider identities', 'ஏற்கனவே உள்ள provider identities')}</strong>
          {!professional && !business ? <p className="summary-note">{text('No existing Professional or Business profile. This is a first-provider application.', 'Professional அல்லது Business profile ஏற்கனவே இல்லை. இது first-provider application.')}</p> : <div className="dashboard-grid" style={{ marginTop: '.65rem' }}>
        ...
        {staleRequestedOwnership ? <p role="alert" style={{ marginTop: '1rem', color: 'var(--danger, #b42318)', fontWeight: 650 }}>{text('This pending application now conflicts with an existing provider identity. The backend approval guard will reject duplicate ownership; review the account state before taking action.', 'இந்த pending application தற்போது existing provider identity-யுடன் conflict ஆகிறது. Duplicate ownership-ஐ backend approval guard block செய்யும்; action எடுக்கும் முன் account state-ஐ review செய்யவும்.')}</p> : null}
        ...
        {application.status === 'pending' ? <div style={{ display: 'grid', gap: '.75rem', marginTop: '1rem' }}><label className="field"><span className="field-label">{t('applications.note')}</span><textarea className="field-control" rows={3} maxLength={1000} value={notes[application.id] ?? ''} onChange={(event) => setNotes((current) => ({ ...current, [application.id]: event.target.value }))} placeholder={t('applications.notePlaceholder')} /></label><div className="button-row"><Button type="button" disabled={busyId === application.id || staleRequestedOwnership} onClick={() => void review(application, 'approve')}>{busyId === application.id ? t('applications.updating') : t('applications.approve')}</Button><Button type="button" variant="quiet" disabled={busyId === application.id} onClick={() => void review(application, 'reject')}>{t('applications.reject')}</Button></div><p className="summary-note">{staleRequestedOwnership ? text('Approval is disabled because the requested provider identity already exists.', 'கோரிய provider identity ஏற்கனவே இருப்பதால் approval disabled.') : t('applications.approvalHelp')}</p></div> : null}
      </Card>;
    })}
  </div>;
}
