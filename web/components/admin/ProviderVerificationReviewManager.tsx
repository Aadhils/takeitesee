    ...
    {visible.map((item) => {
      ...
      return <Card key={item.id}>
        ...
        <div className="section-stack" style={{ marginTop: '1rem' }}>
          ...
          {requestDocuments.length ? requestDocuments.map((document) => <div className="card" key={document.id} style={{ padding: '1rem' }}><div className="section-heading"><div><strong>{document.original_filename}</strong><p className="summary-note">{document.mime_type} · {sizeLabel(Number(document.size_bytes))}</p></div><a className="button button-secondary" href={`/api/super-admin/provider-verifications/documents/${document.id}`} target="_blank" rel="noreferrer">View securely</a></div></div>) : <p className="field-error">No private document uploaded. Approval is blocked.</p>}
        </div>
