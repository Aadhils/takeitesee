  ...
  return <div className="section-stack">
    ...
    {visible.map((item) => {
      ...
      return <Card key={item.id}>
        <div className="section-heading"><div><span className="eyebrow">{item.provider_type} · {item.provider_name}</span><h2>{item.service_name}</h2></div><div className="button-row"><Badge tone={tone(item.status)}>{item.status.replaceAll('_',' ')}</Badge>{!canManage ? <Badge tone="neutral">Read only</Badge> : null}</div></div>
        ...
        {item.status === 'pending' ? <div style={{ display: 'grid', gap: '.75rem', marginTop: '1rem' }}>
          <label className="field"><span className="field-label">Reviewer note</span><textarea className="field-control" rows={3} maxLength={1200} disabled={!canManage} value={notes[item.id] ?? ''} onChange={(event) => setNotes((current) => ({ ...current, [item.id]: event.target.value }))} placeholder={canManage ? 'Optional for approval; required for changes/rejection' : 'Your delegated scope allows viewing this request but not managing it.'} /></label>
          <div className="button-row"><Button type="button" disabled={busyId === item.id || !canManage} onClick={() => void review(item,'approve')}>Approve scope</Button><Button type="button" variant="secondary" disabled={busyId === item.id || !canManage} onClick={() => void review(item,'changes_requested')}>Request changes</Button><Button type="button" variant="quiet" disabled={busyId === item.id || !canManage} onClick={() => void review(item,'reject')}>Reject</Button></div>
          <p className="summary-note">{canManage ? 'Approval maps the service to this application/category/location. The provider still controls final activation after all launch gates pass.' : 'This request is visible inside your delegated scope, but a manage-enabled scope is required to review it.'}</p>
        </div> : null}
      </Card>;
    })}
  </div>;
