...
export function ResetPasswordForm() {
  ...
  const showLabel = locale === 'ta-IN' ? 'Password-ஐ காட்டு' : 'Show password';
  const hideLabel = locale === 'ta-IN' ? 'Password-ஐ மறை' : 'Hide password';
  return <div className="auth-page"><section className="page-intro"><span className="eyebrow">{copy.resetEyebrow}</span><h1>{copy.resetTitle}</h1><p>{copy.resetIntro}</p></section><Card className="auth-card"><span className="badge badge-info">{copy.badge}</span><form onSubmit={submit}><PasswordInput label={copy.resetPassword} autoComplete="new-password" required hint={copy.resetHint} value={password} onChange={(event) => setPassword(event.target.value)} showLabel={showLabel} hideLabel={hideLabel} /><PasswordInput label={copy.resetConfirm} autoComplete="new-password" required value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} showLabel={showLabel} hideLabel={hideLabel} />{error ? <p className="field-error" role="alert">{error}</p> : null}<Button type="submit" loading={submitting}>{copy.resetSubmit}</Button></form></Card></div>;
}
