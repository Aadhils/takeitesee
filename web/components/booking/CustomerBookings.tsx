...
export default function CustomerBookings() {
  ...
  return <div className="bookings-page"><section className="page-intro"><span className="eyebrow">Customer space</span><h1>My bookings</h1><p>Review your upcoming, completed, and cancelled bookings.</p></section>{groups.map((group) => <section className="booking-group" aria-labelledby={`group-${group.title}`} key={group.title}><div className="section-heading"><h2 id={`group-${group.title}`}>{group.title}</h2><span className="results-note">{group.values.length} shown</span></div>{group.values.length ? <div className="booking-grid">{group.values.map((booking) => <Card className="booking-card" key={booking.bookingId}><div className="booking-card-top"><div><span className="eyebrow">{booking.bookingReference}</span><h3>{booking.serviceName}</h3></div><Badge tone={booking.status === 'completed' ? 'success' : booking.status === 'cancelled' ? 'danger' : 'info'}>{booking.status}</Badge></div><p className="card-provider">{booking.providerName ?? (booking.providerType === 'business' ? 'Business provider' : 'Professional provider')}</p><div className="booking-card-meta"><span>{booking.bookingDate}, {booking.startTime}</span><span>{formatMoney({ amount: booking.basePrice, currency: booking.currency })}</span><Badge tone="neutral">{booking.paymentStatus}</Badge></div><Link href={`/bookings/${booking.bookingId}`} className="button button-secondary">View booking details</Link></Card>)}</div> : <Card><EmptyState title={`No ${group.title.toLowerCase()} bookings`}>Bookings in this state will appear here when available.</EmptyState></Card>}</section>)}</div>;
}
