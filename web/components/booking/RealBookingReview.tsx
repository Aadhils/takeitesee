...
export default function RealBookingReview({ service, date, dateLabel, time }: { service: CatalogService; date: string; dateLabel?: string; time: string }) {
  ...
  const handleConfirm = async () => {
    ...
    const draft = {
      ...
      basePrice: service.price.amount,
  ...
  return <div className="booking-review-page"><Breadcrumbs items={[{ label: 'Booking', href: `/services/${service.id}/booking` }, { label: 'Review' }]} /><div className="review-page-heading"><span className="eyebrow">Step 4 of 4 · Review</span><h1>Review your booking</h1><p>Check the details below before continuing.</p></div><div className="review-layout"><main className="review-main"><Card className="review-section"><div className="section-heading"><div><span className="eyebrow">Service selected</span><h2>{service.name}</h2></div><Link href={`/services/${service.id}/booking`} className="text-link">Edit selection</Link></div><dl className="review-details"><div><dt>Provider</dt><dd>{service.providerName}</dd></div><div><dt>Date and time</dt><dd>{dateLabel ?? date}, {time} IST</dd></div><div><dt>Duration</dt><dd>{service.durationMinutes} minutes</dd></div><div><dt>Location</dt><dd>{service.location}</dd></div></dl></Card><Card className="review-section"><span className="eyebrow">A note for the provider</span><h2>Anything they should know?</h2><Input label="Notes or instructions" hint="Optional. Saved with your booking request." placeholder="Add access details or context" value={notes} onChange={(event) => setNotes(event.target.value)} /></Card><Card className="review-section"><span className="eyebrow">Before you continue</span><h2>Cancellation and rescheduling</h2><p className="detail-copy">Your requested time remains subject to provider confirmation. Payment is not collected here.</p><label className="terms-row"><input type="checkbox" checked={termsAccepted} onChange={(event) => setTermsAccepted(event.target.checked)} /><span>I understand the booking will remain subject to provider confirmation and payment is not collected here.</span></label></Card></main><aside className="review-aside"><Card className="review-price"><span className="eyebrow">Price summary</span><strong>{formatMoney(service.price)}</strong><span>Starting price</span></Card><Button type="button" disabled={!termsAccepted || submitting} loading={submitting} onClick={handleConfirm}>Confirm booking</Button>{error ? <p className="field-error" role="alert">{error}</p> : null}<p className="explore-disclaimer">You will be asked to sign in before the booking can be created. No payment is collected here.</p></aside></div></div>;
}
