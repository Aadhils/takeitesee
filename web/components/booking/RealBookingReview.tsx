...
import { getCurrentCustomerAsync, presentationAuthAdapter } from '../../services/auth-adapter';
...
export default function RealBookingReview({ service, date, dateLabel, time }: { service: DiscoveryService; date: string; dateLabel?: string; time: string }) {
  ...
  const attemptId = useRef(typeof crypto !== 'undefined' && 'randomUUID' in crypto ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`);

  const reviewPath = `/services/${service.id}/review?date=${encodeURIComponent(date)}&time=${encodeURIComponent(time)}${dateLabel ? `&dateLabel=${encodeURIComponent(dateLabel)}` : ''}`;
  ...
  const handleConfirm = async () => {
    ...
    setError('');
    const draft = {
      idempotencyKey: `booking-${service.id}-${date}-${time}-${attemptId.current}`,
