...
export function BookingSelection({ service }: { service: DiscoveryService }) {
  ...
  return <section className="booking-flow" aria-labelledby="booking-heading"><StepIndicator currentStep={2} /><div className="booking-flow-header"><span className="eyebrow">Presentation booking flow</span><h1 id="booking-heading">Choose a time that works</h1><p>Review the service and provider, then select from illustrative local availability.</p></div><div className="booking-layout"><div className="booking-controls"><Card><DateSelector availability={discoveryAvailability} selectedDate={selectedDate} onSelect={(date) => { setSelectedDate(date); setSelectedTime(''); }} /><TimeSlotSelector day={day} selectedTime={selectedTime} onSelect={setSelectedTime} /></Card><Button type="button" disabled={!selectedTime} onClick={() => undefined}>Continue to review</Button><p className="explore-disclaimer">The continue control demonstrates the next step only. It does not submit or persist anything.</p></div><BookingSummaryCard service={service} date={selectedTime ? day : undefined} time={selectedTime} /></div></section>;
}
