...
export function DiscoveryFilterFields({ filters, onChange }: { filters: DiscoveryFilters; onChange: (filters: DiscoveryFilters) => void }) {
  ...
  const standardLocations = ['Anywhere', 'Chennai', 'Bengaluru', 'Kochi', 'Remote delivery'];
  const locations = standardLocations.includes(filters.location) ? standardLocations : [filters.location, ...standardLocations];
  return <div className="discovery-filter-fields"><Select label="Category" value={filters.category} onChange={(event) => update('category', event.target.value)}><option value="all">All categories</option>{discoveryCategories.map((category) => <option value={category.slug} key={category.id}>{displayText(category.name)}</option>)}</Select><Select label="Location" value={filters.location} onChange={(event) => update('location', event.target.value)}>{locations.map((location) => <option value={location} key={location}>{location}</option>)}</Select><Select label="Price range" value={filters.price} onChange={(event) => update('price', event.target.value as DiscoveryPriceBand)}>{discoveryPriceBands.map((band) => <option value={band.value} key={band.value}>{band.label}</option>)}</Select><Select label="Rating" value={filters.rating} onChange={(event) => update('rating', event.target.value as DiscoveryRatingFilter)}>{discoveryRatingFilters.map((rating) => <option value={rating.value} key={rating.value}>{rating.label}</option>)}</Select><Select label="Availability" value={filters.availability} onChange={(event) => update('availability', event.target.value as DiscoveryAvailabilityFilter)}><option value="any">Any availability</option><option value="today">Available today</option><option value="tomorrow">Next available tomorrow</option><option value="remote">Remote delivery</option></Select><Select label="Provider type" value={filters.provider} onChange={(event) => update('provider', event.target.value as DiscoveryProviderFilter)}><option value="any">Any provider</option><option value="professional">Professional</option><option value="business">Business</option></Select></div>;
}
