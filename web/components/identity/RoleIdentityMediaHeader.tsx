...
function scopeLabel(scope: IdentityScope, tamil: boolean) { if (scope === 'business') return tamil ? 'Business பிராண்ட்' : 'Business brand'; if (scope === 'professional') return tamil ? 'Professional அடையாளம்' : 'Professional identity'; return tamil ? 'Personal account' : 'Personal account'; }
...
export default function RoleIdentityMediaHeader({ context, displayName, subtitle, meta }: { context: IdentityContext; displayName: string; subtitle: string; meta?: string; }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      } catch (cause) { if (active) setError(cause instanceof Error ? cause.message : 'Unable to load identity media.'); }
  ...
  const chooseUpload = (kind: MediaKind) => {
    const guidance = kind === 'avatar'
      ? (tamil ? 'Profile photo: JPEG, PNG அல்லது WebP · அதிகபட்சம் 6 MB. Square image சிறந்த தோற்றம் தரும்.\n\nதொடரவா?' : 'Profile photo: JPEG, PNG or WebP · max 6 MB. A square image works best.\n\nContinue?')
      : (tamil ? 'Banner: JPEG, PNG அல்லது WebP · அதிகபட்சம் 6 MB. Wide image சிறந்த தோற்றம் தரும்.\n\nதொடரவா?' : 'Banner: JPEG, PNG or WebP · max 6 MB. A wide image works best.\n\nContinue?');
  ...
  const upload = async (kind: MediaKind, file: File | null) => {
    ...
    if (!allowedTypes.has(file.type)) { setError(tamil ? 'JPEG, PNG அல்லது WebP image மட்டும் upload செய்யவும்.' : 'Upload a JPEG, PNG, or WebP image.'); return; }
    if (file.size <= 0 || file.size > maxBytes) { setError(tamil ? 'Image 6 MB அல்லது அதற்கு குறைவாக இருக்க வேண்டும்.' : 'Images must be 6 MB or smaller.'); return; }
    ...
    try {
      ...
      const body = await response.json() as { identity?: IdentityMedia; error?: string };
      if (!response.ok || !body.identity) { await supabase.storage.from(identity.bucket).remove([objectPath]); throw new Error(body.error ?? 'Identity media could not be saved.'); }
      setIdentity(body.identity);
      setNotice(kind === 'avatar' ? (tamil ? 'Profile picture update செய்யப்பட்டது.' : 'Profile picture updated.') : (tamil ? 'Background banner update செய்யப்பட்டது.' : 'Background banner updated.'));
    } catch (cause) { setError(cause instanceof Error ? cause.message : (tamil ? 'Image upload செய்ய முடியவில்லை.' : 'Unable to upload image.')); }
  ...
  const remove = async (kind: MediaKind) => {
    ...
    const confirmed = window.confirm(kind === 'avatar' ? (tamil ? 'Profile picture-ஐ remove செய்யவா?' : 'Remove this profile picture?') : (tamil ? 'Background banner-ஐ remove செய்யவா?' : 'Remove this background banner?'));
    if (!confirmed) return;
    setWorking(kind); setError(''); setNotice('');
    try {
      ...
      setIdentity(body.identity);
      setNotice(kind === 'avatar' ? (tamil ? 'Profile picture remove செய்யப்பட்டது.' : 'Profile picture removed.') : (tamil ? 'Background banner remove செய்யப்பட்டது.' : 'Background banner removed.'));
    } catch (cause) { setError(cause instanceof Error ? cause.message : (tamil ? 'Image remove செய்ய முடியவில்லை.' : 'Unable to remove image.')); }
  ...
  return <section className={styles.shell} aria-label="Profile identity media">
    <div className={`${styles.banner} ${bannerClass}`}>
      ...
      <div className={styles.bannerActions}>
        <button type="button" className={styles.mediaButton} disabled={loading || Boolean(working) || !identity} onClick={() => chooseUpload('banner')}>{working === 'banner' ? 'Uploading…' : hasBanner ? (tamil ? 'Banner மாற்று' : 'Change banner') : (tamil ? 'Banner சேர்' : 'Add banner')}</button>
        {hasBanner ? <button type="button" className={styles.mediaButtonDanger} disabled={Boolean(working)} onClick={() => void remove('banner')}>Remove</button> : null}
      </div>
    </div>
    <div className={styles.content}>
      <div className={styles.avatarWrap}>
        {identity?.avatar_url ? <img className={styles.avatar} src={identity.avatar_url} alt={`${displayName} profile`} /> : <div className={styles.avatarFallback} aria-hidden="true">{initials(displayName)}</div>}
        <button type="button" className={styles.avatarEdit} disabled={loading || Boolean(working) || !identity} onClick={() => chooseUpload('avatar')} aria-label={hasAvatar ? 'Change profile picture' : 'Add profile picture'}>{working === 'avatar' ? '…' : '✎'}</button>
      </div>
