...
export function ProviderAvailabilityManager() {
  ...
  const addBlackout = () => {
    ...
    setSaveState('idle');
    setMessage('Blackout added to this service. Click Save availability to keep it.');
  };
  ...
  const save = async () => {
    ...
    try {
      if (selectedService?.preview || !apiConnected) {
        ...
        setSaveState('saved');
        setMessage('✓ Availability saved on this device. Weekly hours and Blackouts will remain after refresh.');
        return;
      }
      ...
      setAvailability(body.availability as Availability);
      setSaveState('saved');
      setMessage('✓ Availability saved successfully to the testing database.');
    } catch (error) {
      setSaveState('error');
  ...
  const saveLabel = saving ? 'Saving…' : saveState === 'saved' ? 'Saved ✓' : 'Save availability';

  return <ProviderShell active="/provider/schedule">
    <ProviderHeading eyebrow="Availability" title="Schedule" description="Set when each service can be booked, define weekly working windows, and block unavailable periods." action={<Button type="button" onClick={() => void save()} disabled={!availability || saving}>{saveLabel}</Button>} />

    {message ? <div className={`mb-5 rounded-xl border px-4 py-3 text-sm ${saveState === 'error' ? 'border-red-200 bg-red-50 text-red-700' : saveState === 'saved' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-white text-slate-600'}`} role="status" aria-live="polite">{message}</div> : null}
    ...
    <Card><div style={{ padding: 24 }}><div className="mb-5"><span className="eyebrow">Blocked periods</span><h2 className="mt-2 text-xl font-semibold">Blackouts</h2><p className="mt-2 text-sm text-slate-600">Block holidays, leave, or any period that should not accept bookings.</p></div><div className="grid gap-4 md:grid-cols-3"><Input label="Starts" type="datetime-local" value={blackoutDraft.starts_at} onChange={(event) => setBlackoutDraft((current) => ({ ...current, starts_at: event.target.value }))} /><Input label="Ends" type="datetime-local" value={blackoutDraft.ends_at} onChange={(event) => setBlackoutDraft((current) => ({ ...current, ends_at: event.target.value }))} /><Input label="Reason" value={blackoutDraft.reason ?? ''} onChange={(event) => setBlackoutDraft((current) => ({ ...current, reason: event.target.value }))} /></div><div className="mt-3"><Button type="button" variant="secondary" onClick={addBlackout}>Add blackout</Button></div>{availability?.blackout_periods.length ? <div className="mt-5 grid gap-3">{availability.blackout_periods.map((item, index) => <div className="flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between" key={`${item.starts_at}-${index}`}><div><strong>{new Date(item.starts_at).toLocaleString()}</strong><span className="block text-sm text-slate-500">to {new Date(item.ends_at).toLocaleString()} · {item.reason || 'Blocked'}</span></div><Button type="button" variant="quiet" onClick={() => { markChanged(); setAvailability({ ...availability, blackout_periods: availability.blackout_periods.filter((_, i) => i !== index) }); }}>Remove</Button></div>)}</div> : <p className="mt-5 text-sm text-slate-500">No blocked periods configured.</p>}</div></Card>
  </ProviderShell>;
}
