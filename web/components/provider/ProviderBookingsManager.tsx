...
function queueLabel(booking: ProviderBooking, now: number) {
  if (booking.closeout_state === 'closed') return 'finally closed';
  if (booking.closeout_state === 'eligible_to_close') return 'closeout due';
  if (booking.attendance_outcome === 'customer_no_show') return 'customer no-show';
  if (booking.attendance_outcome === 'provider_no_show') return 'provider no-show';
...
export default function ProviderBookingsManager() {
  ...
  const queueOptions: Array<{ key: QueueKey; label: string; count: number }> = [
    ...
    { key: 'outcomes', label: 'Closeout', count: counts.outcomes },
    { key: 'cancelled', label: 'Cancelled', count: counts.cancelled },
    { key: 'all', label: 'All', count: counts.all },
  ];

  return <LiveProviderShell active="/provider/bookings">
    <ProviderHeading eyebrow="Operations" title="Bookings" description="Work from live requests, upcoming jobs, attendance outcomes and final closeout queues." />

    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '.75rem', marginBottom: '1rem' }}>
      {queueOptions.slice(0, 4).map((item) => <Card key={item.key} style={{ padding: '1rem' }}><span className="eyebrow">{item.label}</span><strong style={{ display: 'block', marginTop: '.35rem', fontSize: '1.65rem' }}>{item.count}</strong></Card>)}
    </div>

    <div aria-label="Booking queues" style={{ display: 'flex', flexWrap: 'wrap', gap: '.55rem', marginBottom: '1rem' }}>
      {queueOptions.map((item) => <button key={item.key} type="button" className="button button-secondary" aria-pressed={queue === item.key} onClick={() => setQueue(item.key)} style={queue === item.key ? { borderColor: 'var(--color-primary)', background: 'var(--color-selected)', color: 'var(--color-primary-strong)' } : undefined}>{item.label} <span aria-hidden="true">({item.count})</span></button>)}
    </div>

    <Card style={{ padding: '1rem', marginBottom: '1rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '.8rem', alignItems: 'end' }}>
        <label className="field"><span className="field-label">Search bookings</span><input className="field-control" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Reference, service or location" /></label>
        <Select label="Status" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as 'all' | BookingStatus)}><option value="all">Any status</option><option value="pending">Pending</option><option value="rescheduled">Reschedule request</option><option value="confirmed">Confirmed</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></Select>
        <Select label="Date" value={dateScope} onChange={(event) => setDateScope(event.target.value as DateScope)}><option value="all">All dates</option><option value="today">Today</option><option value="7d">Next 7 days</option><option value="30d">Next 30 days</option></Select>
        <Select label="Payment" value={paymentFilter} onChange={(event) => setPaymentFilter(event.target.value as 'all' | PaymentStatus)}><option value="all">Any payment status</option><option value="unpaid">Unpaid</option><option value="pending">Pending</option><option value="paid">Paid</option><option value="failed">Failed</option><option value="refunded">Refunded</option></Select>
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', gap: '.75rem', alignItems: 'center', marginTop: '.9rem' }}><span className="provider-fixture-note">{visible.length} of {items.length} booking{items.length === 1 ? '' : 's'} shown</span><Button type="button" variant="quiet" onClick={resetView}>Reset view</Button></div>
    </Card>
    ...
    {loading ? <Card><p>Loading provider bookings…</p></Card> : visible.length ? <div className="provider-booking-list">
      {visible.map((booking) => {
        ...
        return <Card className="provider-booking-card" key={booking.id}>
          <div className="provider-booking-top"><div><span className="eyebrow">{booking.booking_reference}</span><h2>{booking.service_name}</h2></div><Badge tone={statusTone(booking, now)}>{queueLabel(booking, now)}</Badge></div>
    ...
    </div> : <Card><EmptyState title={queue === 'action' ? 'Nothing needs action' : 'No bookings match this view'}>{queue === 'action' ? 'New requests, reschedules, and services ready for completion will appear here. No-show outcomes move to Closeout.' : 'Try another queue or reset the filters.'}</EmptyState></Card>}

    <BookingReasonDialog open={Boolean(declineTarget)} eyebrow={declineTarget?.status === 'rescheduled' ? 'Decline new time' : 'Decline booking'} title={declineTarget ? (declineTarget.status === 'rescheduled' ? `Decline the new time for ${declineTarget.service_name}?` : `Decline ${declineTarget.service_name}?`) : 'Decline booking?'} description={declineTarget?.status === 'rescheduled' ? 'Choose why the new time cannot be accepted. Declining this reschedule cancels the booking because the previous slot has already been released.' : 'Choose the clearest reason. It will be saved in the booking lifecycle for support and audit history.'} options={declineTarget?.status === 'rescheduled' ? rescheduleDeclineReasons : declineReasons} confirmLabel={declineTarget?.status === 'rescheduled' ? 'Decline new time' : 'Decline booking'} busy={Boolean(declineTarget && busyId === declineTarget.id)} onClose={() => setDeclineTarget(null)} onConfirm={async (reason) => { if (!declineTarget) return; if (await act(declineTarget.id, 'decline', reason)) setDeclineTarget(null); }} />
  </LiveProviderShell>;
}
