...
type SetupReadiness = { verified: boolean; profile_complete: boolean; marketplace_disclosure_complete: boolean; marketplace_live: boolean; trust_status: TrustStatus; trust_reason?: string | null; services: { id: string; scope_enabled: boolean; launch_ready: boolean; category_name?: string | null; location_name?: string | null }[] };
...
type ActivationState = { ready: boolean; title: string; detail: string; href: string; action: string; tone: 'success' | 'warning' | 'danger' };
...
export function ProviderCatalogManager() {
  ...
  const categoryDisplay = (category: PlatformCategory) => category.parent_id
    ? `${parentById.get(category.parent_id)?.name ?? t('catalog.categoryFallback')} → ${category.name}`
    : category.name;
  ...
  const categoryDisplayByName = (name: string) => {
    ...
    return category ? categoryDisplay(category) : name || t('catalog.categoryRequired');
  };
  ...
  const loadServices = useCallback(async () => {
    try {
      ...
      if (!serviceResponse.ok) throw new Error(servicePayload.error || t('catalog.loadServicesFallback'));
      if (!setupResponse.ok || !setupPayload.readiness || !setupPayload.options) throw new Error(setupPayload.error || t('catalog.loadReadinessFallback'));
      ...
      setError(cause instanceof Error ? cause.message : t('catalog.loadServicesFallback'));
  ...
  const activationState = (service?: SetupReadiness['services'][number]): ActivationState => {
    if (!readiness) return { ready: false, title: t('catalog.activationCheckingTitle'), detail: t('catalog.activationCheckingDetail'), href: '/provider/setup', action: t('catalog.openProviderSetup'), tone: 'warning' };
    if (readiness.trust_status === 'suspended') return { ready: false, title: t('catalog.suspendedTitle'), detail: t('catalog.suspendedDetail'), href: '/account/support', action: t('catalog.openPlatformSupport'), tone: 'danger' };
    if (readiness.trust_status === 'reverification_required') return { ready: false, title: t('catalog.reverifyTitle'), detail: t('catalog.reverifyDetail'), href: '/provider/verification', action: t('catalog.continueReverification'), tone: 'warning' };
    if (!readiness.profile_complete) return { ready: false, title: t('catalog.profileIncompleteTitle'), detail: t('catalog.profileIncompleteDetail'), href: '/provider/profile', action: t('catalog.completeProfileShort'), tone: 'warning' };
    if (!readiness.verified) return { ready: false, title: t('catalog.providerVerificationTitle'), detail: t('catalog.providerVerificationDetail'), href: '/provider/verification', action: t('catalog.openVerificationShort'), tone: 'warning' };
    if (!readiness.marketplace_disclosure_complete) return { ready: false, title: t('catalog.disclosureIncompleteTitle'), detail: t('catalog.disclosureIncompleteDetail'), href: '/provider/verification', action: t('catalog.completeDisclosure'), tone: 'warning' };
    if (!service?.scope_enabled) return { ready: false, title: t('catalog.scopeApprovalTitle'), detail: t('catalog.scopeApprovalDetail'), href: '/provider/setup', action: t('catalog.reviewLaunchScope'), tone: 'warning' };
    if (!service.launch_ready) return { ready: false, title: t('catalog.launchIncompleteTitle'), detail: t('catalog.launchIncompleteDetail'), href: '/provider/setup', action: t('catalog.openProviderSetup'), tone: 'warning' };
    return { ready: true, title: t('catalog.reactivationReadyTitle'), detail: t('catalog.reactivationReadyDetail'), href: '/provider/services', action: t('catalog.reviewService'), tone: 'success' };
  };
  ...
  const openEdit = (item: CatalogItem) => {
    ...
    setFormOpen(true);
    setNotice(category ? '' : t('catalog.legacyCategoryNotice'));
  };
  ...
  const saveDraft = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok || !payload.service) throw new Error(payload.error || t('catalog.saveFallback'));
      setNotice(categoryChanged && editingItem?.status === 'active'
        ? t('catalog.categoryChangedNotice')
        : editingId ? t('catalog.editSavedNotice') : t('catalog.createSavedNotice'));
      closeForm(); await loadServices();
    } catch (cause) { setError(cause instanceof Error ? cause.message : t('catalog.saveFallback')); }
    finally { setSaving(false); }
  };

  const setStatus = async (id: string, status: CatalogStatus) => {
    if (status === 'active' && readiness?.trust_status !== 'normal') { setError(readiness?.trust_status === 'suspended' ? t('catalog.suspendedActivationError') : t('catalog.reverifyActivationError')); return; }
    if (status === 'active' && !readyMap.get(id)?.launch_ready) { setError(t('catalog.notLaunchReadyError')); return; }
    setNotice(''); setError('');
    try {
      ...
      const payload = await response.json() as { service?: ProviderServiceApiRecord; error?: string };
      if (!response.ok || !payload.service) throw new Error(payload.error || t('catalog.statusUpdateFallback'));
      setNotice(`${t('catalog.statusUpdatedPrefix')} ${statusLabel(status)}.`); await loadServices();
    } catch (cause) { setError(cause instanceof Error ? cause.message : t('catalog.statusUpdateFallback')); }
  };
  ...
  return <LiveProviderShell active="/provider/services">
    ...
    {formOpen ? <Card className="mb-6 overflow-hidden"><form onSubmit={saveDraft} style={cardInset} className="grid gap-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><span className="eyebrow">{editingId ? t('catalog.editService') : t('catalog.newService')}</span><h2 className="mt-2 text-2xl font-semibold">{editingId ? t('catalog.updateDetails') : t('catalog.addToCatalog')}</h2></div><Button type="button" variant="quiet" onClick={closeForm}>{t('common.close')}</Button></div><div className="grid gap-4 md:grid-cols-2"><Input label={t('catalog.serviceName')} value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} required /><Select label={t('catalog.platformCategory')} hint={t('catalog.platformCategoryHint')} value={draft.category_id} onChange={(event) => setDraft((current) => ({ ...current, category_id: event.target.value }))} required><option value="" disabled>{t('catalog.selectSpecificCategory')}</option>{selectableCategories.map((category) => <option value={category.id} key={category.id}>{categoryDisplay(category)}</option>)}</Select><Input label={t('catalog.price')} type="number" min="0" value={draft.price} onChange={(event) => setDraft((current) => ({ ...current, price: Number(event.target.value) }))} required /><Input label={t('catalog.duration')} type="number" min="15" step="15" value={draft.duration} onChange={(event) => setDraft((current) => ({ ...current, duration: Number(event.target.value) }))} required /></div><label className="grid gap-2 text-sm font-medium">{t('catalog.about')}<textarea className="min-h-32 w-full rounded-xl border border-slate-200 bg-white px-4 py-3" value={draft.description} onChange={(event) => setDraft((current) => ({ ...current, description: event.target.value }))} /></label><div className="rounded-xl border border-dashed p-4 text-sm text-slate-600">{t('catalog.missingCategoryHelp')} <Link href="/provider/category-requests" className="text-link">{t('catalog.requestCategoryReview')} →</Link></div><div style={actionRowStyle}><Button type="submit" style={actionButtonStyle} loading={saving} disabled={!draft.category_id}>{editingId ? t('catalog.saveChanges') : t('catalog.add')}</Button><Button type="button" variant="secondary" style={outlinedActionStyle} onClick={() => setDraft((current) => ({ ...current, status: 'draft' }))}>{t('catalog.saveDraft')}</Button></div></form></Card> : null}

    <div className="grid gap-5 lg:grid-cols-2">{visibleItems.map((item) => { const serviceReady = readyMap.get(item.id); const activation = activationState(serviceReady); return <Card key={item.id} className="min-w-0 overflow-hidden"><div style={cardInset} className="flex min-w-0 flex-col gap-5"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><span className="eyebrow">{categoryDisplayByName(item.category)}</span><h2 className="mt-2 break-words text-xl font-semibold">{item.name}</h2></div><Badge tone={statusTone(item.status)}>{statusLabel(item.status)}</Badge></div><p className="break-words text-sm leading-6 text-slate-600">{item.description || t('catalog.noDescription')}</p><div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><div className="rounded-xl border" style={metaInset}><span className="block text-xs text-slate-500">{t('catalog.price')}</span><strong>INR {item.price.toLocaleString(locale)}</strong></div><div className="rounded-xl border" style={metaInset}><span className="block text-xs text-slate-500">{t('catalog.duration')}</span><strong>{item.duration} {t('catalog.minutesShort')}</strong></div><div className="rounded-xl border" style={metaInset}><span className="block text-xs text-slate-500">{t('catalog.launchScope')}</span><strong>{serviceReady?.scope_enabled ? `${serviceReady.category_name} · ${serviceReady.location_name}` : t('catalog.approvalRequired')}</strong></div></div>{item.status === 'paused' ? <Alert title={activation.title} tone={activation.tone}>{activation.detail} {!activation.ready ? <Link href={activation.href}>{activation.action} →</Link> : null}</Alert> : null}<ProviderServiceReachControl serviceId={item.id} serviceName={item.name} /><div style={actionRowStyle}><Button type="button" variant="secondary" style={outlinedActionStyle} onClick={() => openEdit(item)}>{t('common.edit')}</Button>{item.status !== 'active' ? <Button type="button" style={actionButtonStyle} disabled={!activation.ready} title={!activation.ready ? activation.detail : undefined} onClick={() => void setStatus(item.id, 'active')}>{item.status === 'paused' ? t('catalog.reactivate') : t('catalog.activate')}</Button> : null}{item.status === 'active' ? <Button type="button" variant="secondary" style={outlinedActionStyle} onClick={() => void setStatus(item.id, 'paused')}>{t('catalog.pause')}</Button> : null}{item.status !== 'draft' ? <Button type="button" variant="secondary" style={outlinedActionStyle} onClick={() => void setStatus(item.id, 'draft')}>{t('catalog.moveDraft')}</Button> : null}{!activation.ready && item.status !== 'active' ? <Link href={activation.href} className="text-link">{activation.action} →</Link> : null}</div></div></Card>; })}</div>
