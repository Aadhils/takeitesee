...
export function ProviderCatalogManager() {
  ...
  const activationState = (service?: SetupReadiness['services'][number]): ActivationState => {
    if (!readiness) return { ready: false, title: tamil ? 'Activation readiness check ஆகிறது' : 'Checking activation readiness', detail: tamil ? 'Provider readiness load ஆன பிறகு service-ஐ மீண்டும் Active செய்ய முடியுமா என்று காணலாம்.' : 'Wait for Provider readiness to load before reactivating this service.', href: '/provider/setup', action: tamil ? 'Provider Setup திறக்க' : 'Open Provider Setup', tone: 'warning' };
    if (readiness.trust_status === 'suspended') return { ready: false, title: tamil ? 'Suspension காரணமாக paused' : 'Paused while Provider is suspended', detail: tamil ? 'Platform restoration முடியும் வரை இந்த service private-ஆகவே இருக்கும். Support request-ஐ review செய்து suspension-ஐ resolve செய்யுங்கள்.' : 'This service stays private until platform restoration is complete. Review your support case and resolve the suspension first.', href: '/account/support', action: tamil ? 'Platform Support திறக்க' : 'Open Platform Support', tone: 'danger' };
    if (readiness.trust_status === 'reverification_required') return { ready: false, title: tamil ? 'Fresh verification தேவை' : 'Fresh verification required', detail: tamil ? 'Re-verification approved ஆகும் வரை service activation கிடையாது.' : 'Service activation stays blocked until fresh re-verification is approved.', href: '/provider/verification', action: tamil ? 'Re-verification தொடர' : 'Continue re-verification', tone: 'warning' };
    if (!readiness.profile_complete) return { ready: false, title: tamil ? 'Profile basics incomplete' : 'Provider profile basics incomplete', detail: tamil ? 'Service launch-க்கு Provider profile basics complete ஆக வேண்டும்.' : 'Complete the Provider profile basics before this service can return to Active.', href: '/provider/profile', action: tamil ? 'Profile complete செய்ய' : 'Complete profile', tone: 'warning' };
    if (!readiness.verified) return { ready: false, title: tamil ? 'Provider verification தேவை' : 'Provider verification required', detail: tamil ? 'Verified Provider identity இல்லாமல் service-ஐ Active செய்ய முடியாது.' : 'A verified Provider identity is required before service activation.', href: '/provider/verification', action: tamil ? 'Verification திறக்க' : 'Open verification', tone: 'warning' };
    if (!readiness.marketplace_disclosure_complete) return { ready: false, title: tamil ? 'Marketplace disclosure incomplete' : 'Marketplace disclosure incomplete', detail: tamil ? 'Public legal, contact மற்றும் grievance details complete ஆன பிறகே service launch-ready ஆகும்.' : 'Complete the public legal, contact, and grievance disclosure before this service is launch-ready.', href: '/provider/verification', action: tamil ? 'Disclosure complete செய்ய' : 'Complete disclosure', tone: 'warning' };
    if (!service?.scope_enabled) return { ready: false, title: tamil ? 'Category / location approval தேவை' : 'Category and location approval required', detail: tamil ? 'இந்த service-க்கு approved marketplace category மற்றும் location scope வேண்டும்.' : 'This service still needs an approved marketplace category and location scope.', href: '/provider/setup', action: tamil ? 'Launch scope review செய்ய' : 'Review launch scope', tone: 'warning' };
    if (!service.launch_ready) return { ready: false, title: tamil ? 'Launch readiness incomplete' : 'Launch readiness incomplete', detail: tamil ? 'Current launch checks இன்னும் complete ஆகவில்லை. Provider Setup-ல் blocker-ஐ review செய்யுங்கள்.' : 'One or more launch checks are still incomplete. Review the current blocker in Provider Setup.', href: '/provider/setup', action: tamil ? 'Provider Setup திறக்க' : 'Open Provider Setup', tone: 'warning' };
    return { ready: true, title: tamil ? 'Manual re-activation ready' : 'Ready for manual reactivation', detail: tamil ? 'Current Provider, trust, disclosure மற்றும் approved scope checks pass ஆகின்றன. நீங்கள் விரும்பினால் இந்த service-ஐ மீண்டும் Active செய்யலாம்; இது automatic ஆக publish ஆகாது.' : 'Provider, trust, disclosure, and approved scope checks pass. You can manually return this service to Active; TakeItEsee will not reactivate it automatically.', href: '/provider/services', action: tamil ? 'Service review செய்ய' : 'Review service', tone: 'success' };
  };
  ...
  const setStatus = async (id: string, status: CatalogStatus) => {
    ...
    if (status === 'active' && !readyMap.get(id)?.launch_ready) { setError('This service is not launch-ready yet. Review the exact blocker shown on this service before activation.'); return; }
    setNotice(''); setError('');
    try {
      ...
      setNotice(tamil ? `சேவை ${statusLabel(status)} நிலைக்கு மாற்றப்பட்டது.` : `Service moved to ${status}.`); await loadServices();
  ...
  return <LiveProviderShell active="/provider/services">
    ...
    {readiness?.trust_status === 'suspended' ? <Alert title={t('catalog.activationSuspended')} tone="danger">{t('catalog.activationSuspendedHelp')} {readiness.trust_reason || ''} <Link href="/account/support">{tamil ? 'Platform Support திறக்க →' : 'Open Platform Support →'}</Link></Alert> : readiness?.trust_status === 'reverification_required' ? <Alert title={t('catalog.reverification')} tone="warning">{t('catalog.reverificationHelp')} <Link href="/provider/verification">{t('catalog.openVerification')}</Link></Alert> : readiness && !readiness.profile_complete ? <Alert title={t('catalog.completeProfile')} tone="warning">{t('catalog.completeProfileHelp')} <Link href="/provider/profile">{t('catalog.completeProfileAction')}</Link></Alert> : readiness && !readiness.verified ? <Alert title={t('catalog.verificationRequired')} tone="warning">{t('catalog.verificationRequiredHelp')} <Link href="/provider/verification">{t('catalog.openVerification')}</Link></Alert> : readiness && !readiness.marketplace_disclosure_complete ? <Alert title={tamil ? 'Marketplace disclosure incomplete' : 'Marketplace disclosure incomplete'} tone="warning">{tamil ? 'Public legal, contact மற்றும் grievance details complete செய்த பிறகே services launch-ready ஆகும். ' : 'Complete public legal, contact, and grievance details before services become launch-ready. '}<Link href="/provider/verification">{tamil ? 'Disclosure complete செய்ய →' : 'Complete disclosure →'}</Link></Alert> : readiness ? <Alert title="Admin-managed categories" tone="success">Providers choose a specific platform category here; Provider Setup keeps that same category locked through approval. <Link href="/provider/setup">{t('catalog.openSetup')}</Link></Alert> : null}
