...
type FinanceSummary = {
  ...
  recovery_offset_applied_minor: number;
...
const emptyFinance: FinanceSummary = {
  ...
  recovery_offset_applied_minor: 0, transferred_minor: 0,
...
export default function ProviderEarningsManager() {
  ...
  return <LiveProviderShell active="/provider/earnings">
    <ProviderHeading eyebrow="Finance" title="Earnings & payouts" description="Track paid service value, platform commission, provider net earnings, settlement holds, payment-risk holds, recovery balances, recovery offsets, and actual payout transfers." />
    ...
    {payload && bookingSummary ? <>
      ...
      {openRecoveries.length ? <Alert tone="warning" title="Outstanding provider recovery balance">A payment reversal affected funds that had already been paid out. Future eligible payout balance will be applied to this recovery first; only the remaining positive balance can be transferred to your payout destination.</Alert> : null}
      ...
      <div className="provider-summary-grid">
        ...
        <ProviderDashboardSummary label="Recovery balance" value={formatMinor(summary.recovery_open_minor, currency)} detail={openRecoveries.length ? 'Applied before future transfers' : 'No outstanding recovery'} tone={openRecoveries.length ? 'warning' : 'info'} />
        <ProviderDashboardSummary label="Recovery applied" value={formatMinor(summary.recovery_offset_applied_minor, currency)} detail="Completed payout offsets" tone="info" />
        <ProviderDashboardSummary label="Transferred" value={formatMinor(summary.transferred_minor, currency)} detail="Actual completed bank / UPI value" />
      ...
      <Card className="provider-profile-card">
        ...
        <dl className="provider-profile-details">
          ...
          <div><dt>Recovery offset applied</dt><dd>{formatMinor(summary.recovery_offset_applied_minor, currency)}</dd></div>
          <div><dt>Actual transferred</dt><dd>{formatMinor(summary.transferred_minor, currency)}</dd></div>
      ...
      {holds.length || recoveries.length ? <Card className="provider-transactions">
        <div className="section-heading"><div><span className="eyebrow">Finance protection</span><h2>Risk holds & recovery</h2></div><Badge tone={openRecoveries.length ? 'warning' : 'info'}>{openRecoveries.length ? 'Recovery outstanding' : 'Finance review'}</Badge></div>
      ...
      <Card className="provider-transactions">
        <div className="section-heading"><div><span className="eyebrow">Payout history</span><h2>Provider payout transfers & recovery offsets</h2></div><Badge tone="info">Gateway reconciled</Badge></div>
        {payouts.length ? <div className="provider-transaction-list">{payouts.map((item) => <div key={item.id}>
          <div><strong>{formatMinor(item.transfer_amount_minor, item.currency)} transferred</strong><span>Provider net {formatMinor(item.provider_net_minor, item.currency)} · Recovery offset {formatMinor(item.recovery_offset_minor, item.currency)} · {item.settlement_count} settlements · {new Date(item.created_at).toLocaleString('en-IN')}{item.transfer_status ? ` · ${item.transfer_status}${item.transfer_status_code ? `/${item.transfer_status_code}` : ''}` : ''}</span></div>
          <strong>{item.transfer_utr || item.external_reference || (item.transfer_amount_minor === 0 ? 'No external transfer' : 'Transfer not sent yet')}</strong>
      ...
      <Card className="provider-transactions">
        ...
        <p className="provider-fixture-note">A payout is marked paid only after verified gateway completion, or when the entire payable amount is safely consumed by an internal recovery offset. Payment disputes and auto-refunds can hold affected balance; post-payout losses are recovered through immutable ledger entries, and only the remaining positive amount can be transferred externally.</p>
      </Card>
    </> : null}
  </LiveProviderShell>;
}
