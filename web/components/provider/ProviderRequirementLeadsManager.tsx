      ...
      {marketplace.proposals.length === 0 ? <Card><p>{t('lead.noneSubmitted')}</p></Card> : marketplace.proposals.map((proposal) => {
        ...
        return <Card id={`provider-proposal-history-${proposal.id}`} tabIndex={targeted ? -1 : undefined} className={`policy-card${targeted ? ' provider-targeted-proposal' : ''}`} key={proposal.id}>
          ...
          {proposal.status === 'accepted' ? <div className="provider-award-next-actions">
            <div><strong>{tamil ? 'Customer உங்களைத் தேர்வு செய்துள்ளார்' : 'The customer chose you'}</strong><p>{tamil ? 'Customer time-ஐ உடனே schedule செய்திருந்தால் booking உங்கள் Bookings workspace-ல் இருக்கும். Time பின்னர் தேர்வு செய்திருந்தால் private chat-ல் details confirm செய்து காத்திருக்கலாம்.' : 'If the customer scheduled a time while choosing you, the booking is already in your Bookings workspace. If they chose to schedule later, use private chat to confirm details and wait for the schedule.'}</p></div>
            <div className="provider-award-next-actions-buttons"><Link className="button button-primary" href="/provider/bookings">{tamil ? 'Bookings பார்க்க' : 'Open bookings'}</Link><Link className="button button-secondary" href={chatHref}>{tamil ? 'Customer-க்கு message செய்' : 'Message customer'}</Link></div>
          </div> : null}
