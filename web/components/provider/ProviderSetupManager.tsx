...
export default function ProviderSetupManager() {
  ...
  const steps = readiness ? [
    ...
    { label: 'Complete marketplace disclosure', done: readiness.marketplace_disclosure_complete, href: '/provider/verification', detail: 'Legal identity, public contact, principal address, and grievance contact' },
    ...
    { label: 'Launch to public marketplace', done: readiness.marketplace_live, href: '/provider/services', detail: `${readiness.services_active} active service${readiness.services_active === 1 ? '' : 's'} with all public gates complete` },
  ] : [];
  ...
  return <LiveProviderShell active="/provider/setup">
    <ProviderHeading eyebrow="Launch readiness" title="Provider setup" description="Finish profile, verification, public disclosure, trust, and canonical service-scope gates before public marketplace launch." />
    ...
    {readiness ? <>
      ...
      {!readiness.marketplace_disclosure_complete ? <Alert title="Public marketplace disclosure required" tone="warning">Your service cannot become customer-visible until legal identity, public contact, principal address, and grievance contact are complete. <Link href="/provider/verification">Complete disclosure →</Link></Alert> : null}
      <Card>
        <div className="section-heading"><div><span className="eyebrow">Onboarding progress</span><h2>{effectiveProgress}% ready</h2></div><Badge tone={readiness.marketplace_live ? 'success' : 'warning'}>{readiness.marketplace_live ? 'Public marketplace live' : 'Setup in progress'}</Badge></div>
      ...
      <section id="service-launch" className="section-stack">
        ...
        {readiness.services.map((service) => {
          ...
          return <Card key={service.id}>
            ...
            {service.scope_enabled && readiness.verified && !readiness.marketplace_disclosure_complete ? <p className="summary-note">Scope is approved. Complete marketplace disclosure before activation.</p> : null}
            ...
            {service.launch_ready && service.status !== 'active' ? <p><Link href="/provider/services" className="text-link">All public launch gates ready — activate this service →</Link></p> : null}
          </Card>;
        })}
      </section>
    </> : null}
