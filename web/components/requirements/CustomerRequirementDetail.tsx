...
function formatMoney(minor: number, currency: 'INR' | 'USD') { return new Intl.NumberFormat('en-IN', { style: 'currency', currency, maximumFractionDigits: 0 }).format(minor / 100); }
function formatBudget(row: RequirementRow) { if (row.budget_type === 'negotiable') return 'Negotiable'; if (row.budget_type === 'fixed') return formatMoney(Number(row.budget_min_minor ?? 0), row.currency); return `${formatMoney(Number(row.budget_min_minor ?? 0), row.currency)} – ${formatMoney(Number(row.budget_max_minor ?? 0), row.currency)}`; }
...
export default function CustomerRequirementDetail({ requirementId }: { requirementId: string }) {
  ...
  const updateStatus = async (status: 'open' | 'paused' | 'fulfilled' | 'cancelled') => {
    ...
    try {
      const response = await fetch(`/api/requirements/${encodeURIComponent(requirementId)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) });
      const payload = await response.json() as { error?: string };
      if (!response.ok) throw new Error(payload.error || 'Requirement could not be updated.');
      setNotice(`Requirement marked ${status}.`); await load();
  ...
  const decideProposal = async (proposalId: string, decision: 'accept' | 'decline') => {
    ...
    try {
      ...
      setNotice(decision === 'accept' ? 'Provider selected. Your private chat and service-job scheduling are now ready.' : 'Proposal declined.'); await load();
  ...
  return <div style={{ display: 'grid', gap: '1rem' }}>
    ...
    <Card className="policy-card">
      ...
      <dl className="review-details"><div><dt>Category</dt><dd>{categoryName || 'Service'}</dd></div><div><dt>Location</dt><dd>{locationName || 'Location'}</dd></div><div><dt>Service mode</dt><dd>{requirement.service_mode}</dd></div><div><dt>Budget</dt><dd>{formatBudget(requirement)}</dd></div><div><dt>Needed by</dt><dd>{requirement.needed_by || 'Flexible'}</dd></div><div><dt>Posted</dt><dd>{new Intl.DateTimeFormat('en-IN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(requirement.published_at))}</dd></div></dl>
      {['open','paused'].includes(requirement.status) ? <div style={{ display: 'flex', gap: '.6rem', flexWrap: 'wrap', marginTop: '1rem' }}>{requirement.status === 'open' ? <Button type="button" variant="quiet" loading={busy} onClick={() => void updateStatus('paused')}>Pause new proposals</Button> : null}{requirement.status === 'paused' ? <Button type="button" variant="secondary" loading={busy} onClick={() => void updateStatus('open')}>Reopen proposals</Button> : null}<Button type="button" variant="secondary" loading={busy} onClick={() => void updateStatus('fulfilled')}>Mark fulfilled</Button><Button type="button" variant="danger" loading={busy} onClick={() => void updateStatus('cancelled')}>Cancel</Button></div> : requirement.status === 'awarded' ? <div style={{ display: 'grid', gap: '.65rem', marginTop: '1rem' }}><div style={{ display: 'flex', gap: '.6rem', flexWrap: 'wrap' }}><Link className="button button-primary" href="/messages">Open private chat</Link><Button type="button" variant="danger" loading={busy} onClick={() => void updateStatus('cancelled')}>Cancel requirement</Button></div><p className="summary-note">Schedule the linked service job below. Fulfillment is automatic after service completion, customer confirmation and payment settlement.</p></div> : <p className="summary-note" style={{ marginTop: '1rem' }}>This requirement is closed and cannot be reopened.</p>}
    </Card>
    ...
    <Card className="policy-card">
      ...
      {proposals.length === 0 ? <p className="detail-copy">No proposals yet. Matching verified providers can respond while this requirement is open.</p> : <div style={{ display: 'grid', gap: '1rem', marginTop: '1rem' }}>
        {proposals.map((proposal) => <div key={proposal.id} style={{ border: '1px solid #e7eaf0', borderRadius: '16px', padding: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '.6rem', flexWrap: 'wrap', alignItems: 'start' }}>{proposal.status === 'submitted' && canReviewProposals ? <><Button type="button" loading={proposalBusyId === proposal.id} onClick={() => void decideProposal(proposal.id, 'accept')}>Accept proposal</Button><Button type="button" variant="quiet" loading={proposalBusyId === proposal.id} onClick={() => void decideProposal(proposal.id, 'decline')}>Decline</Button></> : null}<MarketplaceReportForm targetType="proposal" targetId={proposal.id} label="Report proposal" /></div>
          {proposal.status === 'accepted' ? <div style={{ display: 'flex', gap: '.75rem', alignItems: 'center', flexWrap: 'wrap', marginTop: '.6rem' }}><Link className="button button-secondary" href="/messages">Open private chat</Link><p className="summary-note">Only you and the selected provider can access this conversation.</p></div> : null}
        </div>)}
      </div>}
    </Card>
    ...
    <Card className="policy-card"><span className="eyebrow">Audit history</span><h2>Requirement lifecycle</h2><div style={{ display: 'grid', gap: '.75rem', marginTop: '1rem' }}>{events.map((event) => <div key={event.id} style={{ borderBottom: '1px solid #ececf2', paddingBottom: '.75rem' }}><strong>{event.event_type === 'created' ? 'Requirement posted' : `Status changed to ${event.to_status}`}</strong><p className="summary-note">{event.from_status ? `${event.from_status} → ${event.to_status} · ` : ''}{new Intl.DateTimeFormat('en-IN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(event.created_at))}</p></div>)}</div></Card>
  </div>;
}
