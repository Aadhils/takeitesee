...
-- Super-admin is intentionally separate from operational admin.
...
create table application_locations (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  primary key (application_id, location_id)
);

create table platform_categories (
  id uuid primary key default gen_random_uuid(),
  application_id uuid not null references platform_applications(id) on delete cascade,
  parent_id uuid references platform_categories(id) on delete restrict,
  code text not null,
  name text not null,
  description text,
  active boolean not null default true,
...
-- Maps the existing service catalog into the ecosystem without changing service ownership.
create table service_ecosystem_scope (
  service_id uuid primary key references services(id) on delete cascade,
  application_id uuid not null references platform_applications(id) on delete restrict,
  category_id uuid references platform_categories(id) on delete restrict,
...
-- Admin membership is independent from provider/customer business data.
...
create table admin_scopes (
  ...
  location_id uuid references platform_locations(id) on delete cascade,
  category_id uuid references platform_categories(id) on delete cascade,
  ...
  created_at timestamptz not null default now(),
  check (
    (scope_type = 'platform' and application_id is null and location_id is null and category_id is null and service_id is null)
...
create table admin_audit_log (
  ...
  resource_id text,
  application_id uuid references platform_applications(id) on delete set null,
  location_id uuid references platform_locations(id) on delete set null,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);
...
-- RLS is enabled now; policies are deliberately introduced with authenticated
-- admin helper functions in the next migration before any admin UI is exposed.
