-- Universal Services: harden selected-weekday scheduling for recurring requirement jobs.
-- Recovery reuses customer_create_requirement_job(), so this central gate covers both normal
-- occurrence creation and repeated same-sequence recovery without duplicating recovery logic.
-- Weekly requirements with explicit weekdays must stay on a selected weekday and, when the
-- recurrence interval is greater than one, inside an active recurrence week.
...
begin
  ...
  if requirement_row.schedule_pattern='recurring' then
    ...
    if requirement_row.recurrence_frequency='weekly' and requirement_row.recurrence_weekdays is not null then
      if not (extract(dow from requested_booking_date)::smallint = any(requirement_row.recurrence_weekdays)) then
        raise exception 'The selected booking date must match one of the recurring weekdays.';
      end if;
      if (((requested_booking_date-requirement_row.needed_by)/7) % coalesce(requirement_row.recurrence_interval,1))<>0 then
        raise exception 'The selected booking date is outside the recurring interval week.';
      end if;
    end if;
  else
