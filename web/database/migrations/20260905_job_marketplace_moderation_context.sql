-- Professional ecosystem: job conversation safety + moderation.
--
-- Generalizes the existing marketplace moderation ledger so reports from job
-- application conversations/messages use the same safety queue and audit trail.
-- Existing requirement/proposal moderation semantics remain unchanged.
-- Job moderation is visible/manageable only to platform-scope Admins/Super Admins.
-- No finance, recurrence, payment, payout, settlement, refund or recovery behavior.

alter table public.marketplace_moderation_reports
  add column if not exists context_kind text not null default 'requirement';
...
begin
  if not exists (
    ...
    where conname='marketplace_moderation_reports_job_application_id_fkey'
      and conrelid='public.marketplace_moderation_reports'::regclass
  ) then
    alter table public.marketplace_moderation_reports
      add constraint marketplace_moderation_reports_job_application_id_fkey
      foreign key(job_application_id) references public.job_applications(id) on delete restrict;
  end if;
end $$;

create index if not exists marketplace_moderation_reports_job_application_idx
  on public.marketplace_moderation_reports(job_application_id,status,created_at desc)
...
alter table public.marketplace_moderation_reports
  ...
  check (context_kind in ('requirement','job_application'));
...
comment on column public.marketplace_moderation_reports.context_kind is
  'Safety context: requirement for service marketplace reports; job_application for applicant/employer conversation reports.';
comment on column public.marketplace_moderation_reports.job_application_id is
  'Job application associated with a job conversation/message safety report; null for requirement moderation.';

-- Reporter keeps access to their report. Requirement Admin scope stays category/location
-- aware; job reports are restricted to platform-scope Admin/Super Admin visibility.
...
using (
  ...
  or (
    ...
    and public.admin_can_view(null,null,null,null)
  )
);
...
begin
  ...
  if report_row.context_kind='requirement' and report_row.requirement_id is not null then
    ...
    can_manage:=public.admin_can_manage(null,null,null,null);
  end if;
