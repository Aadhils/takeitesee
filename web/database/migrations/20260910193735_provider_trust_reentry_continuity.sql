-- Provider trust recovery -> marketplace re-entry continuity.
-- Re-verification can clear only through fresh verification approval.
-- Suspension/re-verification pause Business Products just like Services, and restore never auto-reactivates either.
-- Finance/Cashfree/payment/refund/payout/settlement/reconciliation/recovery remain HOLD.
-- Recurrence/recovery remain frozen.

create or replace function private.guard_business_product_active_trust()
returns trigger
language plpgsql
security definer
set search_path = ''
as $$
begin
  if new.status = 'active'::public.business_product_status
     and (tg_op = 'INSERT' or new.status is distinct from old.status)
     and not private.provider_trust_allows_marketplace('business', null, new.business_id)
  then
    raise exception 'Provider trust must be normal before a Business product can be set Active.';
  end if;
  return new;
end;
$$;
revoke all on function private.guard_business_product_active_trust() from public, anon, authenticated;
grant execute on function private.guard_business_product_active_trust() to service_role;

drop trigger if exists business_products_active_trust_guard on public.business_products;
create trigger business_products_active_trust_guard
before insert or update of status on public.business_products
for each row execute function private.guard_business_product_active_trust();
...
begin
  ...
  if target_action='require_reverification' then
    ...
    if previous_status='reverification_required' then
      raise exception 'Re-verification must be cleared by approving a fresh provider verification request.';
    end if;
    if previous_status<>'suspended' then raise exception 'Only a suspended provider can be restored with this action.'; end if;
    next_status:='normal';
    event_value:='restored';
    notification_type:='provider_restored';
    notification_title:='Provider trust access restored';
    notification_body:=reason_value || ' Trust is restored. Review public readiness and manually reactivate the services or products you want to make public.';
    notification_target_path:='/provider/public-readiness';
  end if;
  ...
  if target_action in ('require_reverification','suspend') then
    ...
    if target_provider_type='business' then
      update public.business_products
      set status='paused'::public.business_product_status
      where business_id=target_provider_id
        and status='active'::public.business_product_status;
    end if;
  end if;
