-- Native Service discovery availability / capability weighting.
--
-- Preserve the existing public search result contract and all text/category/location
-- semantics. Add only modest deterministic ranking signals from authoritative
-- Service availability configuration and active fulfillment capabilities.
--
-- Finance/Cashfree/payment/refund/payout/settlement/reconciliation/recovery remain untouched.
-- Recurrence/recovery remains frozen. Leaked-password protection remains untouched.

create or replace function marketplace_internal.resolve_service_search_delivery_signals()
returns table(
  service_id uuid,
  availability_priority integer,
  capability_priority integer
)
language sql
stable
security definer
...
as $function$
  select
    s.id as service_id,
    case coalesce(sa.mode::text, 'on_request')
      when 'always_available' then 8
      when 'scheduled' then 5
      else 2
    end::integer as availability_priority,
    (least(count(fm.service_id), 3::bigint) * 3)::integer as capability_priority
  from public.services s
  left join public.service_availability sa
    on sa.service_id = s.id
  left join public.service_fulfillment_modes fm
    on fm.service_id = s.id
   and fm.active = true
  where s.active = true
    and s.status = 'active'::public.service_status
  group by s.id, sa.mode;
$function$;

revoke all on function marketplace_internal.resolve_service_search_delivery_signals()
  from public, anon, authenticated;
grant execute on function marketplace_internal.resolve_service_search_delivery_signals()
...
begin
  ...
  with review_summary as (
    ...
    select d.service_id, d.availability_priority, d.capability_priority
    from marketplace_internal.resolve_service_search_delivery_signals() d
  ), base as (
    select
      ...
      coalesce(ds.availability_priority, 2) as availability_priority,
      coalesce(ds.capability_priority, 0) as capability_priority,
      s.updated_at,
    ...
    left join delivery ds on ds.service_id=s.id
    ...
    select b.*, (
      ...
      + b.availability_priority
      + b.capability_priority
...
comment on function marketplace_internal.resolve_service_search_delivery_signals() is
  'Internal coarse Service discovery ranking signals: availability configuration (2/5/8) plus active fulfillment capability breadth (0/3/6/9). No raw protected capability rows are returned.';
