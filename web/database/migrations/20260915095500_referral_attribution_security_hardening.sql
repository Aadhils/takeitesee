...
begin
  ...
  if char_length(ref_handle) < 3 or char_length(ref_handle) > 30
     or ref_handle !~ '^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$'
     or ref_handle like '%--%' then
    raise exception 'Invalid referrer handle.' using errcode = '22023';
  end if;

  if char_length(dest_handle) < 3 or char_length(dest_handle) > 30
     or dest_handle !~ '^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$'
     or dest_handle like '%--%' then
    raise exception 'Invalid destination handle.' using errcode = '22023';
  end if;

  if p_landing_path is null
     or char_length(p_landing_path) < 2
     or char_length(p_landing_path) > 500
     or p_landing_path <> '/@' || dest_handle then
...
revoke all on function public.record_public_referral_attribution(text,text,uuid,text)
  from public, anon, authenticated;
grant execute on function public.record_public_referral_attribution(text,text,uuid,text)
  to service_role;

comment on function public.record_public_referral_attribution(text,text,uuid,text) is
  'Server-only finance-free referral attribution write. Service role only; public browser roles must use the controlled application route.';
