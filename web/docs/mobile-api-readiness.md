...
8. Customer booking calendar export and owned reschedule-slot availability are bearer-ready: both read the owned booking with the authenticated Request, and the owned availability path forwards that Request into the shared slot loader while preserving the public availability fallback and conflict semantics.
