...
export const productionBookingRepository: ProductionBookingRepository = {
  async createBooking(session, input) {
    ...
    const { data: existing } = await supabase.from('bookings').select(bookingSelect).eq('idempotency_key', input.idempotency_key).maybeSingle();
  ...
  async rescheduleBooking(session, bookingId, input) {
    ...
    const availabilityInput: CreateBookingInput = { service_id: current.service_id, provider_id: current.provider.provider_id, provider_type: current.provider.provider_type, booking_date: input.booking_date, start_time: input.start_time, timezone: current.timezone, duration_minutes: current.duration_minutes, location: current.location, customer_notes: current.customer_notes, quoted_price: current.quoted_price, currency: current.currency, idempotency_key: `reschedule:${bookingId}:${input.booking_date}:${input.start_time}`, service_name: current.service_name };
    await assertBookingAvailability(availabilityInput, bookingId);
    const supabase = await createSupabaseServerClient();
    const { data, error } = await supabase.rpc('reschedule_owned_booking', { target_booking_id: bookingId, new_booking_date: input.booking_date, new_start_time: input.start_time, reschedule_reason: input.reason ?? null }).maybeSingle();
