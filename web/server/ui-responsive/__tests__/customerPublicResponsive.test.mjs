...
const [identityCss, storefrontCss, mediaHeaderCss, productDetailSource, productDetailShellSource, productDetailCss, storefrontProductsSource, storefrontProductsCss, foundationCss] = await Promise.all([
...
test('product detail and storefront product actions retain existing wrap-safe layout contracts', () => {
  assert.ok(productDetailSource.includes('<ProductDetailShell'));
  assert.ok(productDetailShellSource.includes('className={styles.actionCluster}'));
