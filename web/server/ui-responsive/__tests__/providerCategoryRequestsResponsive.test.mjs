...
test('Provider category requests use a scoped responsive journey wrapper', () => {
  assert.ok(routeSource.includes('ProviderCategoryRequestsResponsive.module.css'));
  assert.ok(routeSource.includes('className={styles.categoryRequestsJourney}'));
...
test('Provider category request controls stay phone and tablet safe', () => {
  ...
  assert.ok(routeSource.includes('className={styles.submitAction}'));
  assert.ok(routeSource.includes('className={styles.requestHeader}'));
  assert.ok(routeSource.includes('className={styles.statusPill}'));
});
...
test('Provider category request governance semantics remain unchanged', () => {
  ...
  assert.ok(routeSource.includes('action={submitProviderCategoryRequest}'));
  assert.ok(routeSource.includes('Requested categories never become public automatically.'));
  assert.ok(routeSource.includes('Pending requests cannot be used for publishing until Super Admin approval creates the canonical category.'));
